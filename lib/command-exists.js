...
const isFile = function (commandName, callback) {
    fs.lstat(commandName, function (err, stats) {
        if (err) {
            callback(!err);
            return;
        }
...
const isFileSync = function (commandName) {
    try {
        const stats = fs.lstatSync(commandName);
        return stats.isFile();
    } catch (e) {
        return false;
    }
...
const commandExistsUnix = function (commandName, cleanedCommandName, callback) {
    ...
    isFile(commandName, function (isFile) {
...
const commandExistsUnixSync = function (commandName, cleanedCommandName) {
    if (isFileSync(commandName) && localExecutableSync(commandName)) {
        return true;
    }
    try {
